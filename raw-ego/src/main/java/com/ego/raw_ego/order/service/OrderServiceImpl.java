...
import com.ego.raw_ego.coupon.service.CouponApplicationResult;
 ...
 *
 * <h3>Checkout pipeline (CRITICAL ΓÇö read before modifying)</h3>
 * <ol>
 ...
 * </ol>
 *
 * <p>Steps 3ΓÇô5 are inside a single {@code @Transactional} boundary.
...
public class OrderServiceImpl implements OrderService {
    ...
    /** Redis lock TTL ΓÇö matches ego.cart.checkout-lock-ttl-minutes in application.properties. */
    ...
    // ΓöÇΓöÇ Checkout lock helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Checkout ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    public OrderDetailResponse checkout(User user, CheckoutRequest request) {
        // ΓöÇ1. Checkout double-submit guard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    /**
     * Internal checkout implementation ΓÇö called within the Redis lock guard.
    ...
    public OrderDetailResponse doCheckout(User user, CheckoutRequest request) {
        ...
        for (CartItemResponse item : cart.getItems()) {
            if (item.getStockStatus() == com.ego.raw_ego.catalog.entity.InventoryRecord.StockStatus.OUT_OF_STOCK) {
                throw new ConflictException(
                        "Item \"" + item.getProductName() + " ΓÇô " + item.getVariantLabel() +
        ...
        // 4. Validate and apply coupon (if provided) ΓÇö inside this transaction so any
        ...
        // 6. Persist OrderCoupon audit record and increment currentUses ΓÇö same transaction.
        if (appliedCoupon != null) {
            couponService.recordUsage(appliedCoupon, savedOrder.getId(), discountAmount);
        }

        // 7. Clear cart (inside same transaction ΓÇö Redis failure = full rollback)
        ...
        // 8. Publish event AFTER saveAndFlush ΓÇö order is committed before async listener reads it.
        //    The async listener runs on ego-async-* threads (Phase 8 ΓÇö SendGrid notification).
    ...
    // ΓöÇΓöÇ Customer reads ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
     ...
     * is thrown ΓÇö not a 403 ΓÇö to avoid leaking order existence.
     *
    ...
    // ΓöÇΓöÇ Customer cancel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    public OrderDetailResponse cancelOrder(Long orderId, Long userId) {
        ...
        // ΓöÇΓöÇ Snapshot items into a plain List BEFORE any restore() call ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        // Hibernate's 1st-level cache ΓÇö including `order` ΓÇö making it detached.
        ...
        // ΓöÇΓöÇ Re-fetch a fresh managed Order after em.clear() ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Admin operations ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    /**
     * Admin: returns all orders, optionally filtered by status (newest first).
     *
     * @param status   optional status filter ΓÇö null returns all orders
    ...
    public OrderDetailResponse adminUpdateStatus(Long orderId, UpdateOrderStatusRequest request) {
        ...
        // ΓöÇΓöÇ Persist shipment tracking info when courier has received the order ΓöÇΓöÇΓöÇΓöÇ
        ...
        // new ones ΓÇö a null request field leaves the existing DB value untouched.
        ...
        // ΓöÇΓöÇ Publish status-change notification events ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        // Events are published AFTER saveAndFlush() ΓÇö the DB record is committed
        ...
        log.info("Order status advanced: orderId={} ΓåÆ {} trackingNumber={} courier={}",
                orderId, request.getStatus(), order.getTrackingNumber(), order.getCourierName());
        return toDetailResponse(savedOrder);
    }

    // ΓöÇΓöÇ Mappers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    private Order buildOrder(User user, CheckoutRequest request, CartResponse cart,
        ...
        BigDecimal grandTotal = cart.getSubtotal()
                ...
                .add(BigDecimal.ZERO) // shippingTotal ΓÇö always ZERO for now
                .max(BigDecimal.ZERO); // cannot go negative

        // ΓöÇΓöÇ Resolve shipping address ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        if (request.getAddressId() != null) {
            // Load saved address and snapshot it to JSON ΓÇö changes to the address book
     ...
     * The format is intentionally simple ΓÇö all values are escaped for basic injection safety.
     */
