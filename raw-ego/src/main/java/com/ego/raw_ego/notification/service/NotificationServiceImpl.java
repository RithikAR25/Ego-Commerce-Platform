...
/**
 * Core email notification service ΓÇö builds and dispatches transactional emails
 * via the SendGrid Web API.
 *
 * <h3>Architecture rules observed</h3>
 * <ul>
 *   <li>NOT {@code @Transactional} ΓÇö SendGrid HTTP calls must never hold a DB connection.</li>
 *   <li>All DB reads use {@code findByIdWithUserAndItems} (JOIN FETCH) so the User and
 *       items are fully initialized before the session closes ΓÇö safe on any async thread.</li>
 *   <li>Errors are caught internally ΓÇö failures logged to {@code notification_logs},
 ...
 * <p>All HTML is built by direct {@code StringBuilder.append()} calls ΓÇö dynamic values are
...
public class NotificationServiceImpl implements NotificationService {
    ...
    // ΓöÇΓöÇ Public send methods ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    /**
     * Sends an order confirmation email when a customer places an order
     * (status: {@code PENDING_PAYMENT}).
     *
     * <p>Checks idempotency first ΓÇö if a SUCCESS log already exists for this
    ...
    public void sendOrderConfirmation(Long orderId) {
        if (notificationLogService.alreadySent(orderId, NotificationEventType.ORDER_PLACED)) {
            log.info("[Notification] ORDER_PLACED already sent for orderId={} ΓÇö skipping", orderId);
            return;
        }

        // JOIN FETCH loads User + items in one query ΓÇö prevents LazyInitializationException
    ...
    public void sendPaymentConfirmation(Long orderId) {
        if (notificationLogService.alreadySent(orderId, NotificationEventType.PAYMENT_CONFIRMED)) {
            log.info("[Notification] PAYMENT_CONFIRMED already sent for orderId={} ΓÇö skipping", orderId);
            return;
        }
        ...
        String recipientEmail = order.getUser().getEmail();
        String subject = "Payment confirmed ΓÇö EGO order #" + orderId + " is being prepared";
     ...
     *
     * <p>Idempotency is NOT checked here ΓÇö re-sends are explicitly triggered by the user.
    ...
    public void sendOrderShipped(Long orderId) {
        if (notificationLogService.alreadySent(orderId, NotificationEventType.ORDER_SHIPPED)) {
            log.info("[Notification] ORDER_SHIPPED already sent for orderId={} ΓÇö skipping", orderId);
            return;
        }
    ...
    public void sendOrderDelivered(Long orderId) {
        if (notificationLogService.alreadySent(orderId, NotificationEventType.ORDER_DELIVERED)) {
            log.info("[Notification] ORDER_DELIVERED already sent for orderId={} ΓÇö skipping", orderId);
            return;
        }
     ...
     * Includes the Razorpay refund ID and expected credit timeline (5ΓÇô7 business days).
     *
    ...
    public void sendRefundCompleted(Long orderId, String razorpayRefundId) {
        if (notificationLogService.alreadySent(orderId, NotificationEventType.REFUND_COMPLETED)) {
            log.info("[Notification] REFUND_COMPLETED already sent for orderId={} ΓÇö skipping", orderId);
            return;
        }
        ...
        String recipientEmail = order.getUser().getEmail();
        String subject        = "Your EGO refund has been processed ΓÇö order #" + orderId;
     ...
     * a generic reference key ΓÇö acceptable at this scale.
     *
    ...
    // ΓöÇΓöÇ Core dispatch ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    public void sendWishlistOutOfStock(com.ego.raw_ego.auth.entity.User user,
        ...
        String subject  = "Heads up ΓÇö \"" + productName + "\" is now out of stock";
    ...
    public void sendWishlistBackInStock(com.ego.raw_ego.auth.entity.User user,
                                        Long productId, String productName) {
        String subject  = "Back in stock ΓÇö \"" + productName + "\" is available again!";
     ...
     * All exceptions are caught ΓÇö failures never propagate to the async listener.
     */
    ...
    // ΓöÇΓöÇ HTML builders ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    //
    ...
    // here ΓÇö any literal '%' in CSS (e.g. width="100%") or in user data will throw
    // UnknownFormatConversionException at runtime.

    private String buildOrderPlacedHtml(Order order) {
        StringBuilder sb = new StringBuilder();

        // ΓöÇΓöÇ Doctype + head + body open ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        // ΓöÇΓöÇ Outer wrapper table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        sb.append("<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background:#0f0f0f;padding:40px 0;\">");
        sb.append("<tr><td align=\"center\">");

        // ΓöÇΓöÇ Card table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        for (OrderItem item : order.getItems()) {
            sb.append("<tr style=\"border-bottom:1px solid #252525;\">")
              ...
              .append("<td style=\"padding:14px 12px;color:#c8a96e;font-size:13px;font-weight:600;text-align:right;\">Γé╣").append(formatMoney(item.getLineTotal())).append("</td>")
              .append("</tr>");
        }
        ...
        sb.append("<tr>")
          ...
          .append("<td style=\"padding:8px 0;color:#ffffff;font-size:13px;text-align:right;\">Γé╣").append(formatMoney(order.getSubtotal())).append("</td>")
          .append("</tr>");
        sb.append("<tr>")
          .append("<td style=\"padding:8px 0;color:#888;font-size:13px;\">Shipping</td>")
          .append("<td style=\"padding:8px 0;color:#ffffff;font-size:13px;text-align:right;\">Γé╣").append(formatMoney(order.getShippingTotal())).append("</td>")
          .append("</tr>");
        sb.append("<tr style=\"border-top:1px solid #2a2a2a;\">")
          ...
          .append("<td style=\"padding:14px 0 0;color:#c8a96e;font-size:15px;font-weight:700;text-align:right;\">Γé╣").append(formatMoney(order.getGrandTotal())).append("</td>")
          .append("</tr>");
        sb.append("</table>");
        ...
        sb.append("<tr><td style=\"padding:24px 40px;background:#111111;text-align:center;\">")
          ...
          .append("<p style=\"margin:6px 0 0;color:#555;font-size:11px;\">This is an automated message ΓÇö please do not reply.</p>")
          .append("</td></tr>");
    ...
    private String buildPaymentConfirmedHtml(Order order) {
        ...
        sb.append("<td style=\"padding:12px 0;border-bottom:1px solid #2a2a2a;\">")
          ...
          .append("<span style=\"color:#c8a96e;font-size:16px;font-weight:700;\">#").append(order.getId()).append("</span>")
          .append("</td>");
        String paymentId = order.getRazorpayPaymentId() != null ? order.getRazorpayPaymentId() : "ΓÇö";
        sb.append("<td style=\"padding:12px 0;border-bottom:1px solid #2a2a2a;text-align:right;\">")
        ...
        sb.append("<tr style=\"border-top:1px solid #2a2a2a;\">")
          ...
          .append("<td style=\"padding:14px 0;color:#4caf7d;font-size:15px;font-weight:700;text-align:right;\">Γé╣").append(formatMoney(order.getGrandTotal())).append("</td>")
          .append("</tr>");
        sb.append("</table>");
    ...
    private String formatMoney(BigDecimal amount) {
        ...
        // Use BigDecimal formatting directly ΓÇö avoids String.format() with its
    ...
    // ΓöÇΓöÇ HTML builders: shipping / delivery / refund ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    private String buildOrderShippedHtml(Order order) {
        ...
        sb.append("<p style='margin:0 0 32px 0;color:#555;font-size:15px;line-height:1.6;'>Great news ΓÇö your EGO order <strong>#").append(order.getId()).append("</strong> has been shipped and is on its way to you. You can expect delivery within <strong>3ΓÇô5 business days</strong>.</p>");
        ...
        for (OrderItem item : order.getItems()) {
            ...
            sb.append("<span style='float:right;font-size:14px;color:#111;font-weight:700;'>├ù").append(item.getQuantity()).append("</span>");
            sb.append("</td></tr>");
        }
        sb.append("</table>");
        ...
        sb.append("<p style='margin:0;color:#555;font-size:14px;'>ΓÇö The EGO Team</p>");
        sb.append("</td></tr>");
    ...
    private String buildOrderDeliveredHtml(Order order) {
        ...
        // Review CTA
        sb.append("<p style='margin:0 0 24px 0;color:#555;font-size:15px;line-height:1.6;'>Share your thoughts ΓÇö your review helps other shoppers.</p>");
    ...
    private String buildRefundCompletedHtml(Order order, String razorpayRefundId) {
        ...
        sb.append("<p style='margin:0 0 32px 0;color:#555;font-size:15px;'>Hi ").append(escapeHtml(firstName)).append(",</p>");
        sb.append("<p style='margin:0 0 32px 0;color:#555;font-size:15px;line-height:1.6;'>We've successfully processed your refund for order <strong>#").append(order.getId()).append("</strong>. The amount will be credited back to your original payment method within <strong>5ΓÇô7 business days</strong>, depending on your bank.</p>");
        ...
        sb.append("<span style='float:right;font-size:14px;color:#111;font-weight:700;'>5ΓÇô7 Business Days</span>");
        sb.append("</td></tr>");
        sb.append("</table>");

        sb.append("<p style='margin:0;color:#888;font-size:13px;line-height:1.6;'>If you have any questions about your refund, contact your bank with the refund reference above. We're sorry to see you go ΓÇö we hope to serve you again.</p>");
        sb.append("</td></tr>");
    ...
    private String buildWishlistStockHtml(String firstName, String productName, boolean backInStock) {
        ...
        String headline     = backInStock
                ? "Great news ΓÇö it's back!"
                : "One of your wishlist items is out of stock";
        String bodyText     = backInStock
                ? "\"" + escapeHtml(productName) + "\" is available again. Don't miss out ΓÇö sizes sell fast."
