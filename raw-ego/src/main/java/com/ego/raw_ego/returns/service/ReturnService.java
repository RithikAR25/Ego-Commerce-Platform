 ...
 *
 * <h3>Return flow (CRITICAL — read before modifying)</h3>
 * <ol>
 ...
 *         <li>Approve: Razorpay refund API called (OUTSIDE @Transactional — per
 *             ARCHITECTURE_RULES.md). On success: status → REFUND_COMPLETED, order →
 ...
 *       boundary — keeps DB connections free during external HTTP call.</li>
 ...
 *       method — same as the Phase 6 cancellation flow.</li>
 * </ul>
 */
@Service
@Slf4j
...
public class ReturnService {
    ...
    /** Return window in days — customers have this many days from delivery to submit a return. */
     ...
     *       {@code DELIVERED} status history entry — NOT {@code order.updatedAt}).</li>
    ...
    public ReturnRequestResponse initiateReturn(User user, Long orderId, InitiateReturnRequest request) {
        // 1. Load order WITH statusHistory eagerly — required to extract the DELIVERED timestamp
        ...
        // 3. Guard: return window check — 7 days from the DELIVERED status transition.
        ...
        Instant deliveredAt = order.getStatusHistory().stream()
                ...
                .max(Instant::compareTo)   // use the latest DELIVERED entry (defensive — normally only one)
     ...
     *
     * @param status   optional status filter — null returns all requests
     ...
     * <p>Immediately sets status → {@code REJECTED}. No Razorpay call. Order unchanged.
     *
     * <h3>Approval path (CRITICAL — read before modifying)</h3>
     * <p>Steps:
     * <ol>
     *   <li>Validate refundAmount &gt; 0 and ≤ order.grandTotal.</li>
     *   <li>Set status → {@code APPROVED}, persist (outside @Transactional).</li>
     *   <li>Load order with items for inventory restoration data.</li>
     *   <li>Call Razorpay refund API ({@link #callRazorpayRefund}) — <b>OUTSIDE @Transactional</b>.</li>
     *   <li>On success: call {@link #completeRefundAndUpdateOrder} — <b>inside @Transactional</b>:
     *       <ul>
     *         <li>Store {@code razorpayRefundId} on the return request.</li>
     *         <li>Set return status → {@code REFUND_COMPLETED}.</li>
     *         <li>Advance order status → {@code REFUNDED}.</li>
    ...
    public ReturnRequestResponse adminReviewReturn(Long returnId, AdminReviewReturnRequest request) {
        // 1. Load return request — no @Transactional yet
        ReturnRequest returnRequest = returnRepository.findById(returnId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Return request not found: id=" + returnId));
        ...
        // 4. Load order with items — need grandTotal for cap validation + items for inventory restore
        ...
        if (request.getRefundAmount().compareTo(order.getGrandTotal()) > 0) {
            throw new ConflictException(
                    ...
                    " exceeds order grand total ₹" + order.getGrandTotal() + ".");
        }
        ...
        // 8. Call Razorpay refund API — OUTSIDE @Transactional (per ARCHITECTURE_RULES.md)
        ...
        // 9. All DB writes after successful Razorpay call — inside @Transactional
    ...
    /**
     * Persists a rejection decision. {@code @Transactional} — single DB write.
     */
    @Transactional
     ...
     * an audit record — the admin can retry.
     */
    @Transactional
     ...
     *
     * <p>MUST be called OUTSIDE any {@code @Transactional} boundary — per ARCHITECTURE_RULES.md:
     ...
     * @param amountInPaise     refund amount in paise (₹1 = 100 paise)
    ...
    private String callRazorpayRefund(String razorpayPaymentId, long amountInPaise, Long returnId) {
        ...
        // ── Live mode: real Razorpay API call ─────────────────────────────────
        try {
            JSONObject refundOptions = new JSONObject();
            refundOptions.put("amount", amountInPaise);
            refundOptions.put("speed", "normal"); // "normal" = 5–7 business days, "optimum" = instant
            ...
            throw new ConflictException(
                    ...
                    "The return request is marked APPROVED — please retry. Details: " + e.getMessage());
        }
    }
     ...
     * <ul>
     *   <li>Return status → {@code REFUND_INITIATED} → {@code REFUND_COMPLETED}</li>
     *   <li>{@code razorpayRefundId} stored on the return request</li>
     *   <li>Order status → {@code REFUNDED}</li>
    ...
    protected ReturnRequestResponse completeRefundAndUpdateOrder(
        ...
        // Each restore() call uses @Modifying(clearAutomatically=true) → em.clear()
        ...
        // ── Re-fetch fresh managed entities after em.clear() ──────────────────
        ...
        // saveAndFlush() forces immediate DB roundtrip — ensures @CreationTimestamp
        ...
        // ── Publish REFUND_COMPLETED notification event ────────────────────────
