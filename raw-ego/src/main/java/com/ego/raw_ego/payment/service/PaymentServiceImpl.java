 ...
 *
 * <h3>Payment flow (CRITICAL ΓÇö read before modifying)</h3>
 * <ol>
 *   <li>Customer places EGO order ΓåÆ {@code PENDING_PAYMENT}.</li>
 *   <li>Frontend calls {@code POST /api/v1/payments/razorpay/create} ΓåÆ
 ...
 *   <li>Customer pays ΓåÆ Razorpay fires {@code POST /api/v1/webhooks/razorpay}.</li>
 ...
 * <ul>
 *   <li>Razorpay HTTP calls are made OUTSIDE {@code @Transactional} ΓÇö per ARCHITECTURE_RULES.md.</li>
 *   <li>Only the DB write in {@link #handleWebhook} is inside a transaction.</li>
 *   <li>The webhook handler is idempotent ΓÇö duplicate events are silently ignored.</li>
 * </ul>
 */
@Service
@Slf4j
...
public class PaymentServiceImpl implements PaymentService {
    ...
    // ΓöÇΓöÇ Create Razorpay order ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
     ...
     * </ul>
     *
     * <p><b>NOT inside a transaction</b> ΓÇö the Razorpay API call must not hold a DB connection.
     *
     * @param egoOrderId the EGO order to create a payment for
     * @param userId     the authenticated user ΓÇö used for ownership check
    ...
    public PaymentOrderResponse createRazorpayOrder(Long egoOrderId, Long userId) {
        // Load EGO order ΓÇö validates existence + ownership
        ...
        // Amount in paise (Γé╣1 = 100 paise) ΓÇö Razorpay requires integer paise
    ...
    // ΓöÇΓöÇ Webhook handler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
     ...
     * (mapped to 400 Bad Request). Returning 400 is intentional ΓÇö Razorpay only retries
     * on 5xx, so a 400 signals "this request is rejected, do not retry".
     *
     * <h3>Idempotency ΓÇö duplicate deliveries return 200 OK</h3>
     ...
     * always returns {@code 200 OK} ΓÇö the correct production-grade response for duplicate
    ...
    public void handleWebhook(String rawBody, String razorpaySignature) {
        // 1. Verify HMAC-SHA256 signature ΓÇö MANDATORY before any logic
     ...
     * specifying a charset ΓÇö these use the JVM platform default, which varies by OS.
    ...
    private void verifyWebhookSignature(String rawBody, String razorpaySignature) {
        try {
            ...
            if (!computedSignature.equals(razorpaySignature)) {
                log.warn("[Webhook HMAC] Signature mismatch ΓÇö request rejected");
    ...
    private com.razorpay.Order callRazorpayCreateOrder(Long egoOrderId, long amountInPaise) {
        try {
            ...
            // receipt is a short reference label shown in Razorpay dashboard ΓÇö max 40 chars
    ...
    public void confirmOrder(String razorpayOrderId, String razorpayPaymentId) {
        ...
        // Idempotency guard ΓÇö Razorpay may fire the same webhook more than once.
        ...
        if (order.getStatus() == OrderStatus.CONFIRMED) {
            log.info("[Webhook] Duplicate delivery detected (idempotent skip): "
                    + "razorpayOrderId={} orderId={} ΓÇö already CONFIRMED, returning 200 OK",
        ...
        // Publish event AFTER saveAndFlush ΓÇö CONFIRMED status + razorpay_payment_id
